...
def load_checkpoint(
    ...
    if "epoch" in checkpoint.keys() and not epoch_reset:
        ...
        # ✅ Fine-tune 模式：只加载模型，不加载优化器
        if cfg is not None and hasattr(cfg, "TRAIN") and getattr(cfg.TRAIN, "CHECKPOINT_LOAD_MODEL_ONLY", False):
            print("[INFO] Fine-tune mode detected, skipping optimizer/scaler state loading.")
        else:
