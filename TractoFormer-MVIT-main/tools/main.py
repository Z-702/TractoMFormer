...
def load_config(args):
    ...
    if hasattr(args, "data_num"):
        cfg.DATA_NUM = int(args.data_num)
    if hasattr(args, "data_aug_num"):
        cfg.DATA_AUG_NUM = int(args.data_aug_num)
    if hasattr(args, "data_mode"):
...
def main():
    ...
    cfg.MODEL.NUM_CLASSES = 2 #添加的强制使其变为2分类
    cfg = assert_and_infer_cfg(cfg)
    out_dir = pathlib.Path(cfg.OUTPUT_DIR)
    result_root = out_dir.parent
    result_name = out_dir.name
    for n_fold in range(5): # 5-fold cross-validation
        cfg.N_FOLD = n_fold
        new_name = f'{result_name}_{n_fold}'
        (result_root/new_name).mkdir(exist_ok=True)
        cfg.OUTPUT_DIR = str(result_root/new_name)
        # out_dir = pathlib.Path(cfg.OUTPUT_DIR)
