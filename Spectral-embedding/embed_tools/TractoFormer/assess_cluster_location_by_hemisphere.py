...
def main():
    ...
    for fname, c_idx in zip(input_polydatas, list(range(len(input_polydatas)))):
        ...
        if outdir is not None:
            ...
            if location_data[c_idx][1] == b'h':

                fname_output = os.path.join(outdir, fname_base.replace('.vtp', '_right.vtp'))
                wma.io.write_polydata(pd_right, fname_output)
                fname_output = os.path.join(outdir, fname_base.replace('.vtp', '_left.vtp'))
                wma.io.write_polydata(pd_left, fname_output)
            else:
                fname_output = os.path.join(outdir, fname_base.replace('.vtp', '_comm.vtp'))
