...
datafolder=/data01/embed-tmp/HCP/data-dsl40/
outdir=/data01/embed-tmp/HCP/tractoembedding
mkdir -p $outdir

while read subID
do
  echo $subID
  tractvtk=$(ls $datafolder/$subID*vtp)
  echo $tractvtk
  ...
  if [ ! -f $embedtract ]; then
    $1 /home/fan/Software/miniconda3/py310_23.3.1-0/envs/DeepWMA/bin/python \
    ...
    $1 /home/fan/Software/miniconda3/py310_23.3.1-0/envs/tractoformer/bin/python \
...
#    $1 rm $embeddingfolder/tracts/${subID}_featMatrix.h5
#    $1 rm -r $embeddingfolder/tracts/${subID}_prediction_tracts_outlier_removed/
#    $1 rm -r $embeddingfolder/tracts/${subID}_separated/
#    $1 rm $embeddingfolder/tracts/${subID}_multi_tract_specific_prediction_mask.h5
  fi
  ...
  if [ ! -f $embeddiffusionmeasure ]; then
      $1 /home/fan/Software/miniconda3/py310_23.3.1-0/envs/tractoformer/bin/python \
            /home/fan/Projects/TractoFormer/extract_measures_tensors_CNP.py \
            -inputVTK $embedtract \
            -outputNPY $embeddiffusionmeasure
  fi

  if [ ! -f $embeddingfolder/tractoembedding/da-full/fiber_indices.npy ]; then
      $1 /home/fan/Software/miniconda3/py310_23.3.1-0/envs/tractoformer/bin/python \
...
done < /data01/embed-tmp/HCP/sublist.txt