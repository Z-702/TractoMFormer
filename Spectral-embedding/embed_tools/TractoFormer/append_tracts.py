#!/home/fan/Software/miniconda3/py310_23.3.1-0/bin/python
import os
import argparse
...
def main():
    ...
    for c_idx in range(len(cluster_vtp_list)):
        ...
        tract_list.append(numpy.ones(pd_cluster.GetNumberOfLines())*c_idx)
        tract_name.append(os.path.basename(cluster_vtp).replace('.vtp', ''))
    ...
    tract_list = numpy.concatenate(tract_list)
    print(tract_list.shape)
    print(tract_name)

    with h5py.File(outputfile.replace('.vtp', '.h5'), "w") as f:
        f.create_dataset('tract_list', data=tract_list)
        f.create_dataset('tract_name', data=tract_name)
