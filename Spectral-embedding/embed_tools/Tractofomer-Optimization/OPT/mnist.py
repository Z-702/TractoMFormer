...
class Fiber(data.Dataset):
    ...
    def __getitem__(self, index: int):
        fiber = self.ds[index]
        fiber = torch.tensor(fiber.T, dtype=float)
        return fiber
...
class Fiber_Emb(data.Dataset):
    def __init__(self, wbc, emb, transform=None):
        self.wbc = wbc
        self.emb = emb
        self.lth = len(wbc)
        self.transform = transform
    ...
    def __getitem__(self, index: int):
        fiber = self.wbc[index]
        embed = self.emb[index]

        fiber = torch.tensor(fiber.T, dtype=float)
        embed = torch.tensor(embed.T, dtype=float)
...
class n_Fiber_pair(data.Dataset):
    def __init__(self, vec, transform=None):
        # vec=np.reshape(vec,(len(ds),-1,ds.shape[1]))
        self.vec = vec  # 8000*14*3
        self.transform = transform
...
class MNIST(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False, small=False,
                 full=False):
